...
void getPic(list *o_O, char *dir){/*getpicture begin*/   
        long long i=o_O->name;
        char *boom =malloc(MAXLEN);
        sprintf(boom,"%spic/jpg%lld.jpg",dir,i);/*the file name*/
        ...
        while(1){/*loop to read through the section*/
           ...
           if(c == EOF){/*at the end of the real hdd*/
             break;
           }
           putc(c,fp);
           j=j+1;
           if(j==2){/*check to see if the fifth byte is 0x00*/
             if(c!=0x00){
               break;
             }
           }
...
/*static long long memA;
static long long  iii; 
void isIt(){
  memA=0;
  iii=0;
}
